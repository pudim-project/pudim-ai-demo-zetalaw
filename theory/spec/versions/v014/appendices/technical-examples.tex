...
\emph{Source context.} This result is used in the cited application chain \cite{abel-gawronski-neuschel}.
\end{counterexample}
\begin{proof}
Set
\[
...
\emph{Source context.} This result is used in the cited application chain \cite{qi-hlambda}.
\end{counterexample}
\begin{proof}
...
Thus the degree-four transforms required by Qi's conjecture are not completely monotone on the conjectured source ranges. The conjecture in Remark 7.4 is false. \cite{qi-hlambda}.
\end{proof}
...
\emph{Source context.} This result is used in the cited application chain \cite{app-app-0035-source}.
\end{proposition}
\begin{proof}
...
\emph{Source context.} This result is used in the cited application chain \cite{app-app-0004-source,yin-pk-digamma}.
\end{proposition}
\begin{proof}
\emph{Setup.}
...
Complete monotonicity would imply nonnegativity at order zero. Hence \(H_2(\cdot;\alpha_L)\) is not completely monotone for these fixed parameters \(\alpha_L\in(0,1/2)\). The source's universal Turan-window statement is false already in the \(n=2\) upper window. \cite{mishra-swaminathan-ramanujan}.
...
\emph{Source context.} This result is used in the cited application chain \cite{app-app-0009-source}.
\end{proposition}
\begin{proof}
\emph{Setup.}
...
\(x\ge8\).  It remains to prove a single sign change of \(N_0\) on \((1,8]\). \cite{app-app-0009-source}.
...
proof. \cite{app-app-0009-source}.
...
\emph{Source context.} This result is used in the cited application chain \cite{app-app-0004-source}.
\end{proposition}
\begin{proof}
...
\emph{Source context.} This result is used in the cited application chain \cite{kim-song-tail}.
\end{proposition}
\begin{proof}
...
The following appendix result records the fixed source problem \textbf{APP-0037} \cite{du-wang-h3} without interrupting the main exposition.
\begin{proposition}[APP-0037: Du-Wang \(h\_3\) is increasing exactly on \(\frac12\le a\le1\)]
...
\emph{Source reference.} \cite{du-wang-h3}.
\end{proposition}
\begin{proof}
For \(t>1/2\), define
\[
...
\frac12\le a\le1,
\]
and it is not monotone on the two outer windows. \cite{du-wang-h3}.

This proves the claim.
the corresponding result true by source-problem classification.
\end{proof}

The following appendix result records the fixed source problem \textbf{APP-0018} \cite{qi-guo-2004} without interrupting the main exposition.
\begin{proposition}[APP-0018: Qi-Guo tau threshold has a supremum-corrected exact value]
...
\emph{Source reference.} \cite{qi-guo-2004}.
\end{proposition}
\begin{proof}
...
The following appendix result records the fixed source problem \textbf{APP-0023} \cite{sokal-gs} without interrupting the main exposition.
\begin{proposition}[APP-0023: Sokal generalized-Stieltjes lambda-derivatives have a triangular nonnegative compression]
...
\emph{Source reference.} \cite{sokal-gs}.
\end{proposition}
\begin{proof}
...
The following appendix result records the fixed source problem \textbf{APP-0036} \cite{abel-gawronski-neuschel} without interrupting the main exposition.
...
\emph{Source reference.} \cite{abel-gawronski-neuschel}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:baskakov-alpha1-r8-not-cm}.
\end{counterexample}
\begin{proof}
Set
\[
...
The following appendix result records the fixed source problem \textbf{APP-0020} \cite{qi-hlambda} without interrupting the main exposition.
...
\emph{Source reference.} \cite{qi-hlambda}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:qi-hlambda-x4-source-range-not-cm}.
\end{counterexample}
\begin{proof}
Near zero,
\[
...
conjecture fails. \cite{qi-hlambda}.
\end{proof}

The following appendix result records the fixed source problem \textbf{APP-0013} \cite{yin-pk-digamma} without interrupting the main exposition.
...
\emph{Source reference.} \cite{yin-pk-digamma}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:yin-pk-digamma-near-zero-asymptotic}.
\end{corollary}
\begin{proof}
\emph{Setup.}
Fix \(p\in\mathbb N\) and \(k>0\). Define
...
The following appendix result records the fixed source problem \textbf{APP-0039} \cite{mishra-swaminathan-ramanujan} without interrupting the main exposition.
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:ramanujan-turan-n2-upper-window-counterfamily}.
\end{counterexample}
\begin{proof}
Put
\[
...
The following appendix result records the fixed source problem \textbf{APP-0031} \cite{baricz-turan} without interrupting the main exposition.
...
\emph{Source reference.} \cite{baricz-turan}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:baricz-gamma-quotient-a2b3-not-bf}.
\end{counterexample}
\begin{proof}
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:bz-gamma-quotient-n-single-crossing-one-eight}.
\end{proposition}
\begin{proof}
\emph{Setup.}
Use the notation from the previous Bulboaca--Zayed pass:
...
The source itself proves the decreasing part on \((-1,1)\), and the earlier
the preceding theorem proves \(F'(x)>0\) for
\(x\ge8\).  It remains to prove a single sign change of \(N_0\) on \((1,8]\). \cite{app-app-0009-source}. See \ref{res:bz-gamma-quotient-n-single-crossing-one-eight}.
...
The following appendix result records the fixed source problem \textbf{APP-0011} \cite{app-app-0004-source} without interrupting the main exposition.
...
\emph{Source reference.} \cite{app-app-0004-source}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:q2-analytic-denominator-reduced-assembly-terminal-certificate}, \ref{res:q2-compact-taylor-monotonicity-j-terminal-certificate}, \ref{res:q2-remaining-finite-middle-log-cover-terminal-certificate}.
\end{corollary}
\begin{proof}
This roll proves a true auxiliary finite-middle outside-cover node:
...
The following appendix result records the fixed source problem \textbf{APP-0019} \cite{wakrim-w} without interrupting the main exposition.
...
\emph{Source reference.} \cite{wakrim-w}.
Wakrim's source \cite{wakrim-w} supplies the non-Bernstein obstruction for \(\beta>1\); the present proof supplies the remaining Bernstein range \(0\le\beta\le1\). No complete-Bernstein or subordination conclusion is asserted here.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:wakrim-w-symbol-exact-bf-range}.
\end{corollary}
\begin{proof}
Wakrim's source \cite{wakrim-w} supplies the non-Bernstein obstruction for \(\beta>1\); the present proof supplies the remaining Bernstein range \(0\le\beta\le1\). No complete-Bernstein or subordination conclusion is asserted here.
...
subordination conclusion is asserted here. \cite{wakrim-w}.
...
remaining case and gives the exact Bernstein range. \cite{wakrim-w}.
\end{proof}

The following appendix result records the fixed source problem \textbf{APP-0006} \cite{kim-song-tail} without interrupting the main exposition.
...
\emph{Source reference.} \cite{kim-song-tail}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:zeta-tail-s9-reusable-certificate}.
\end{corollary}
\begin{proof}
For integer \(s>1\), the Hurwitz-tail Euler--Maclaurin expansion has the form
\[
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:baskakov-r4-alpha1-laplace-density-seed}.
\end{proposition}
\begin{proof}
For \(\alpha=1\),
\[
...
The following appendix result records the fixed source problem \textbf{APP-0033} \cite{mishra-swaminathan-ramanujan} without interrupting the main exposition.
...
The following appendix result records the fixed source problem \textbf{APP-0034} \cite{app-app-0009-source} without interrupting the main exposition.
...
\emph{Source reference.} \cite{app-app-0009-source}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:bz-gamma-quotient-critical-window-reduction}.
\end{corollary}
\begin{proof}
\emph{Setup.}
Use the notation from the previous Bulboaca--Zayed pass:
...
The source itself proves the decreasing part on \((-1,1)\), and the earlier
the preceding theorem proves \(F'(x)>0\) for
\(x\ge8\).  It remains to prove a single sign change of \(N_0\) on \((1,8]\). \cite{app-app-0009-source}. See \ref{res:bz-gamma-quotient-critical-window-reduction}.
...
The following appendix result records the fixed source problem \textbf{APP-0015} \cite{sibisi-prabhakar} without interrupting the main exposition.
...
\emph{Source reference.} \cite{sibisi-prabhakar}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:prabhakar-q-stable-subordination-normal-form}.
\end{corollary}
\begin{proof}
