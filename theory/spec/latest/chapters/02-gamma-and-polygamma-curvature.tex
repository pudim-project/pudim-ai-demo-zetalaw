...
\emph{Source context.} This result is used in the cited application chain \cite{app-app-0004-source,baricz-turan-thesis}.
\end{proposition}
\begin{proof}
...
\emph{Source context.} This result is used in the cited application chain \cite{chen-choi-gurland}.
\end{proposition}
\begin{proof}
Let
\[
...
\]
Therefore \(\log F\) is strictly completely monotone on \((0,\infty)\), proving Chen-Choi Conjecture 1. \cite{chen-choi-gurland}.
\end{proof}
...
\emph{Source context.} This result is used in the cited application chain \cite{wakrim-w}.
\end{proposition}
\begin{proof}
...
The map \(s\mapsto s^a\) is Bernstein for \(0<a<1\). The standard composition theorem says that if \(f\) is completely monotone and \(g\) is Bernstein, then \(f\circ g\) is completely monotone. Hence \(\Phi_{\alpha,\beta}'\) is completely monotone, and \(\Phi_{\alpha,\beta}\) is Bernstein for \(0<\beta\le1\). The endpoint \(\beta=0\) is \(\Phi_{\alpha,0}(s)=s^\alpha\), also Bernstein. \cite{wakrim-w}.
...
The next result applies the preceding method to the fixed source problem \textbf{APP-0021} \cite{szabo-cm}.
\begin{proposition}[APP-0021: Szabo's cutoff problem has \(\alpha\_0=1\)]
...
\emph{Source reference.} \cite{szabo-cm}.
\end{proposition}
\begin{proof}
...
non-increasing, so no \(\alpha>1\) is admissible. \cite{szabo-cm}.
\end{proof}
...
The next result applies the preceding method to the fixed source problem \textbf{APP-0010} \cite{yin-zhang-k-beta}.
...
\emph{Source reference.} \cite{yin-zhang-k-beta}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:cm-laplace-moment-ratio-monotonicity}.
\end{corollary}
\begin{proof}
...
\]
we get the precise monotonicity law: \cite{yin-zhang-k-beta}.

if \(n\) is odd, \(R_{k,n}\) is strictly increasing on \((0,\infty)\);
if \(n\) is even, \(R_{k,n}\) is strictly decreasing on \((0,\infty)\).

This solves the Yin--Zhang Nielsen \(k\)-beta derivative-ratio open problem in the parity-refined form. \cite{yin-zhang-k-beta}.
\end{proof}
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:baricz-vq-q-laplace-normal-form}.
\end{proposition}
\begin{proof}
Put \(u=t^2-x^2\). Then \(t=(u+x^2)^{1/2}\) and
\[
...
The next result applies the preceding method to the fixed source problem \textbf{APP-0022} \cite{chen-choi-gurland}.
...
\emph{Source reference.} \cite{chen-choi-gurland}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:chen-choi-gurland-euler-sum-normal-form}.
\end{corollary}
\begin{proof}
Set
\[
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:prabhakar-q-density-mixture-formula}.
\end{proposition}
\begin{proof}
...
The next result applies the preceding method to the fixed source problem \textbf{APP-0012} \cite{baricz-turan-thesis}.
...
\emph{Source reference.} \cite{baricz-turan-thesis}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:baricz-vq-q-complete-monotonicity}.
\end{corollary}
\begin{proof}
Put \(u=t^2-x^2\).  Then
\begin{equation}
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:baricz-vq-strict-q-logconvexity}.
\end{proposition}
\begin{proof}
Put \(u=t^2-x^2\). Then \(t=(u+x^2)^{1/2}\) and
\[
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:simon-gamma-quotient-bf-alpha-window-open-problem}, \ref{res:simon-gamma-quotient-complement-cm}.
\end{theorem}
\begin{proof}
For \(0<\alpha<1\), put
\[
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:simon-gamma-quotient-derivative-positive-laplace-density}.
\end{proposition}
\begin{proof}
Source artifacts:
