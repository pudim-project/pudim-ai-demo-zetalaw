...
Derivative hierarchies are often too large to handle one inequality at a time.  The determinant and moment-ratio method compresses the hierarchy into a normal form whose positivity propagates through levels, which is especially useful for Mills-ratio, Bessel-type, and related recursive estimates.

The strategy is to isolate the structural determinant statement first, prove the moment-ratio bridge next, and only then derive the application-level bounds.  Numerical or finite-middle certificates may still be necessary, but they belong in the technical appendix unless they introduce a reusable mechanism.
\begin{theorem}
...
\emph{Source context.} This result is used in the cited application chain \cite{app-app-0042-source,from-mills}.
\end{theorem}
\begin{proof}
...
\emph{Source context.} This result is used in the cited application chain \cite{app-app-0040-source}.
\end{theorem}
\begin{proof}
...
\emph{Source context.} This result is used in the cited application chain \cite{yin-zhang-k-beta}.
\end{proposition}
\begin{proof}
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:from-mills-laplace-cm-normal-form}.
\end{theorem}
\begin{proof}
For the standard normal Mills ratio, the normalizing constants cancel:
\[
...
The next result applies the preceding method to the fixed source problem \textbf{APP-0040} \cite{app-app-0040-source}.
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:yt-bessel-w-general-zero-partial-fraction}.
\end{corollary}
\begin{proof}
Put \(\mu=\nu+1>0\). The canonical product for the modified Bessel function gives, locally uniformly in \(z\),
\[
...
The next result applies the preceding method to the fixed source problem \textbf{APP-0032} \cite{from-mills}.
...
\emph{Source reference.} \cite{from-mills}.

\emph{Dependency.} The proof uses the preceding result(s) \ref{res:from-mills-all-l-moment-ratio-quadratic-bound}.
\end{corollary}
\begin{proof}
Set
\[
...
This is the determinant-to-bound extractor. It is a single theorem for every \(L\ge0\). \cite{from-mills}.
...
The next result applies the preceding method to the fixed source problem \textbf{APP-0016} \cite{from-mills}.
...
\emph{Dependency.} The proof uses the preceding result(s) \ref{res:from-mills-all-l-alternating-r-bound-family}.
\end{corollary}
\begin{proof}
