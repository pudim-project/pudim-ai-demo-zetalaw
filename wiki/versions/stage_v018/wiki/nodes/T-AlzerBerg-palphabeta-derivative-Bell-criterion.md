---
id: "T-AlzerBerg-palphabeta-derivative-Bell-criterion"
type: "lemma"
title: "Alzer-Berg p alpha beta derivative Bell criterion"
status: "proved"
tags: ["alzer-berg", "bell-polynomial", "bridge-result", "complete-monotonicity", "derivative-chain", "lemma", "proved", "true"]
parents: ["D-AlzerBerg-EulerLimit-palphabeta-Language", "D-Determinant-triangular-compression-language", "O-AlzerBerg-EulerLimit-palphabeta-CM-source-gate"]
...
# Lemma: Alzer-Berg p alpha beta derivative Bell criterion

## Statement

Let \(F_{\alpha,\beta}(x)=(1+\alpha/x)^{x+\beta}=e^{\Phi(x)}\), where \(\Phi(x)=(x+\beta)\log(1+\alpha/x)\). Since \(p_{\alpha,\beta}(\infty)=0\), \(p_{\alpha,\beta}\) is completely monotone if and only if \(F_{\alpha,\beta}'\) is completely monotone. Equivalently, the Bell-polynomial sign family \((-1)^{n+1}B_n(\Phi',\ldots,\Phi^{(n)})\ge0\) holds for all \(n\ge1\) and \(x>0\).
...
`alzer-berg`, `bell-polynomial`, `bridge-result`, `complete-monotonicity`, `derivative-chain`, `lemma`, `proved`, `true`
