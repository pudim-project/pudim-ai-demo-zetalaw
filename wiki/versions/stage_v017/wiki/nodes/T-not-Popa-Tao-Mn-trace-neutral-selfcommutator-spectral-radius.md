---
id: "T-not-Popa-Tao-Mn-trace-neutral-selfcommutator-spectral-radius"
type: "negative_result"
title: "T-not-Popa-Tao-Mn-trace-neutral-selfcommutator-spectral-radius"
status: "proved"
tags: ["negative_result", "proved"]
parents: ["T-Popa-Tao-M3-trace-neutral-self-commutator-normal-form", "T-Finite-matrix-spectral-certificate-principle"]
refs: ["private Oracle response", "private proof note", "wiki/notes/tao-popa-finite-selfcommutator-Mn.md"]
---

# Negative_Result: T-not-Popa-Tao-Mn-trace-neutral-selfcommutator-spectral-radius

## Statement

The formula inf{||V||^2 : H=VV^*-V^*V}=max(lambda_max(H),-lambda_min(H)) for trace-zero Hermitian H is false in general finite dimension.

## Scope

- Finite trace-neutral self-commutator model; not a solution of Tao's infinite B(H) model.

## Dependencies

- [[wiki/nodes/T-Popa-Tao-M3-trace-neutral-self-commutator-normal-form|Popa-Tao M3 trace-neutral self-commutator normal form]]
- [[wiki/nodes/T-Finite-matrix-spectral-certificate-principle|Finite matrix spectral certificate principle]]

## Proof and provenance references

- `private Oracle response`
- `private proof note`
- `wiki/notes/tao-popa-finite-selfcommutator-Mn.md`

## Tags

`negative_result`, `proved`
